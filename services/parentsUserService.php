...
public function add_child($decode_data)
{
 $parent_id 			   =  $decode_data->userid;
   $query =mysql_query("INSERT INTO `user`(`name`,`dob`,`gender`,`sport`) VALUES('$decode_data->name','$decode_data->dob','$decode_data->gender','$decode_data->sport')");
       if($query)
       {
...
{
	$query = mysql_query("SELECT `userid`, IFNull(`userid`,'') AS userid,IFNull(`name`,'') AS name,IFNull(`dob`,'') AS dob ,IFNull(`gender`,'') AS gender,IFNull(`sport`,'') AS sport FROM `user` WHERE `userid`= $child_id ");
	$num = mysql_num_rows($query);
	 if ($num>0)
	 {
