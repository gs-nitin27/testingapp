...
{
$n=1;
$query =mysql_query("SELECT ji.`userid` AS employerid, ji.`id` AS job_id, uj.`userid` AS applicant_id ,us.`name` AS applicant_name ,us.`user_image` AS applicant_image ,us.`prof_name` AS prof_name ,  us.`prof_id` AS prof_id, us.`sport` AS sport  FROM `gs_jobInfo` AS ji LEFT JOIN `user_jobs` AS uj ON ji.`id` = uj.`userjob` LEFT JOIN `user` AS us ON us.`userid` = uj.`userid` WHERE ji.`userid`='$userid' AND ji.`id`='$jobid' AND uj.`status`>='$n'");
if(mysql_num_rows($query)>0)
{
while($row = mysql_fetch_assoc($query))
{
...
<p style="color:#5666be;font-family:Helvetica Neue,Helvetica,Arial,sans-serif;font-size:16px;font-weight:400;line-height:24px;padding-top:0;margin-top:0;text-align:left">  Please click on the link to reset the password <br>
 ...
 <a href="http://getsporty.in/user_reg/index.php?id='.$id. '">continue reading.</a>
