...
if(mysql_num_rows($query)> 0)
{
  while ($row = mysql_fetch_assoc($query)) {
        ...
        // echo "SELECT `id`,`image` FROM `".$table."` WHERE ".$row['search_para']."";die;
         ...
         if(@$criteria->sport != '')
          {
            ...
            $title = $new_record." new ".$topic." updates on ".$criteria->sport."";
            $message = "Browse and get more";
          }

          else
          { $title = $new_record." new ".$topic." found";
            $message = "New updates for your subcription";
            $sport = '';
          }
        ...
        if($new_record > 0)
        {
                 ...
                 $message1 = array('title'=> @$title , 'message'=>@$message ,'sport'=>@$sport,'location'=>@$criteria->location, 'device_id' => @$row['L_device_id'] , 'type'=>@$row['Moudule'] ,'indicator' => 12);
