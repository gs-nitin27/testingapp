...
$where[]      = ' 1=1 ';
   ...
   $whereclause = implode('AND', $where);
   $req = new liteservice();
   $res = $req->delSubs($userid , mysql_real_escape_string($whereclause),$module); 
   if($res != 0)
   {
   $data = array('status'=> $res , 'data'=>'Record is Deleted'); 
   }else
   {
    $data = array('status'=> $res , 'data'=>'Record is Not is  Exist');
   }
  echo json_encode($data);
}
...
/****************** Modify the Subscribe and Alert*****************/
...
$new_topic         = $_REQUEST['new_topic_of_artical'];
 ...
 $where[]          = ' 1=1 ';
   ...
   if($sports != '')
   {
    ...
    if($sports != '')
    {
      $new_where[] = " `sport` = '$new_sports' ";
      $arr['sport'] = $new_sports;
    }
    else
      {
      $new_where[] = " `sport` = '$new_sports' ";
    ...
    if($location != '')
    {
      $new_where[] = " `location` = '$new_location' ";
      $arr['location'] = $new_location;
    }
    else
    {
      $new_where[] = " `location` = '$new_location' ";
     ...
     if($topic != '')
    {
      $new_where[] = " `topic_of_artical` = '$new_topic' "; 
      $arr['topic_of_artical'] = $new_topic;    
    }
     else
     {
       $new_where[] = " `topic_of_artical` = '$new_topic' "; 
    ...
    if($key != '')
    {
      $new_where[] = " `Description` LIKE '%$new_key%'' ";
     $arr['key'] = $new_key;    
    }
    else
    {
     ...
     $arr['key'] = $new_key;  
      ...
      $new_whereclause = implode('AND', $new_where);
      $whereclause = implode('AND', $where);
   $req = new liteservice();
   $res = $req->modifySub($userid ,mysql_real_escape_string($whereclause), mysql_real_escape_string($new_whereclause),json_encode($arr),$module); 
   if($res != 0)
   {
